...
pokemon_platinum_location = {}
pokemon_platinum_location.__index = pokemon_platinum_location
...
function pokemon_platinum_location.new(name)
    local self = setmetatable({}, pokemon_platinum_location)
    if name then
...
function pokemon_platinum_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = pokemon_platinum_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function pokemon_platinum_location:connect_two_ways(exit, rule)
...
function pokemon_platinum_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function pokemon_platinum_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function pokemon_platinum_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function pokemon_platinum_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function pokemon_platinum_location:accessibility()
...
-- 
function pokemon_platinum_location:discover(accessibility, keys)
...
entry_point = pokemon_platinum_location.new("entry_point")
